...
export class Store {
                ...
                case 'slideIn': {
                    ...
                    if (editorElement.type === 'text' && animation.properties.textType === 'character') {
                        ...
                        editorElement.properties.splittedTexts = getTextObjectsPartitionedByCharacters(
                            (editorElement as any).fabricObject,
                            editorElement
                        );
    ...
    setSelectedElement(selectedElement: EditorElement | null) {
        ...
        if (this.canvas) {
            if (selectedElement?.fabricObject) this.canvas.setActiveObject(selectedElement.fabricObject);
    ...
    updateEditorElement(editorElement: EditorElement) {
        this.setEditorElements(
            this.editorElements.map((element) => (element.id === editorElement.id ? editorElement : element))
        );
    }
        ...
        this.editorElements
            ...
            .forEach((element) => {
                ...
                if (isHtmlVideoElement(video)) {
                    const videoPositionOnTimeLineInSeconds = (this.currentTimeInMs - element.timeFrame.start) / 1000;

                    const cropFromStartTimeInSeconds = element.crop?.fromStart! / 1000;

                    video.currentTime = videoPositionOnTimeLineInSeconds + cropFromStartTimeInSeconds;
