...
export const ScaleRangeInput = ({ height, backgroundColor }: ScaleRangeInputProps) => {
    ...
    const onMouseClickOrMoveToNewPosition = (e: React.MouseEvent<HTMLDivElement>) => {
        if (!isMouseDownRef.current || !containerRef.current || canvasWidth <= 0) return;
        ...
        const normalizedValue = Math.max(0, Math.min(maxTime, timeValueWithOutZoomX));
    ...
    return (
        ...
        >
            ...
            >
                <canvas ref={canvasRef} className="absolute top-0 left-0 bg-white" style={{ display: 'block' }} />
